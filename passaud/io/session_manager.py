...
class SessionManager:
    ...
    def _generate_session_key(self, user_password=None) -> bytes:
        ...
        key_file = self.config_manager.get('DEFAULT', 'session_key_file', 
                                         fallback=os.path.expanduser("~/.passaud_session.key"))
    ...
    def _validate_file_path(self, path: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"File path not in allowed directories: {path}")
            return False
        except Exception as e:
    ...
    def clear_sensitive_data(self):
        ...
        if hasattr(self, 'session_key') and self.session_key:
            ctypes.memset(ctypes.addressof(ctypes.c_char.from_buffer(self.session_key)), 0, len(self.session_key))
